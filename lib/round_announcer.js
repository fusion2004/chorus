...
class RoundAnnouncer {
  ...
  async process() {
    ...
    let [firstSong] = this.roundManager.songs;
  ...
  speech(song, firstTrack) {
    ...
    let msg = builder.create('speak', { headless: true }).ele('amazon:domain', { name: 'conversational' });
    msg.ele('break', { time: '1500ms' });
    if (firstTrack) {
      msg.txt(`Welcome to the listening party for ${this.roundManager.title}.`);
      msg.ele('break');
    }
