...
const RoundAnnouncer = require('./round_announcer');
...
async function playCurrentSongMessage({ channel, currentSong, songs, round }) {
  ...
  let length = formatDuration(currentSong.metadata.format.duration);
  ...
  // discord.js v12
  await channel.send(
    `Now Playing: ${currentSong.safeTitle} by ${currentSong.safeArtist} [${length}]`,
    { embed }
  );
