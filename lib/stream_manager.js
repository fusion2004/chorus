...
class StreamManager extends EventEmitter {
  constructor(roundId, initialSongIndex) {
    ...
    this.roundManager = new RoundManager(roundId, initialSongIndex);
  ...
  async start() {
    ...
    while (this.roundManager.nextSong()) {
      let { current } = this.roundManager;

      if (this.stopped) {
        break;
      }

      await this.playSong(current);
    }
  ...
  async playIntro() {
    this.fileStream = fs.createReadStream('./audio/intro01.mp3', { highWaterMark: 65536 });
    this.shoutStream = new ShoutStream(this._shout);

    this.emit('intro');
    try {
      await pipeline(this.fileStream, this.shoutStream, { signal: this.ac.signal });
    } catch (error) {
  ...
  async playSong(current) {
    this.fileStream = fs.createReadStream(current.song.path(announcerFinal), {
      highWaterMark: 65536,
    });
    this.shoutStream = new ShoutStream(this._shout);

    this.emit('playing', current);
    ...
    // If we stop a stream in the announcer for an entry, don't continue
    if (this.stopped) {
      return;
    }

    this.fileStream = fs.createReadStream(current.song.path(transcodeFinal), {
      highWaterMark: 65536,
    });
    this.shoutStream = new ShoutStream(this._shout);
